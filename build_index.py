...
def index_vectors(similarity_type, dims=None ,num_vecs=None, vecs=None):
        ...
        elif similarity_type == "ivfi":
            index = IVFIndex(vecs, range(len(vecs)))
        else:
...
class LSHIndex():
    ...
    def build(self, num_bits=16):
        self.index = faiss.IndexLSH(self.dimension, num_bits)
        self.index.add(self.vectors)
        
    def query(self, vectors, k=300):
        distances, indices = self.index.search(vectors, k) 
        # I expect only query on one vector thus the slice
        return np.array([self.labels[i] for i in indices[0]])
...
class IVFIndex():
    ...
    def build(self):
        ...
        nlist = 100 if self.vectors.shape[0] > 100 else 2
